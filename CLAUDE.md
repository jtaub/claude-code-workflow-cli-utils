...
1. **`/specs <issue-url>`** — Requirements analyst. Fetches issue via `gh` CLI, asks clarifying questions, then rewrites the spec with testable acceptance criteria. Never discusses implementation.
...
3. **`/create-pr`** — Creates PR from current branch to main via `gh` CLI. Uses structured body format (Added/Changed/Fixed sections). Warns if diff is too large to be a single PR.
...
- Skills use the **`gh` CLI** for all GitHub interactions (fetching issues, creating PRs, reading comments)
